...
<div class="card notice-good"><b>✓ TASK ADDED.</b><?php if((int)($_GET['before_photos_added'] ?? 0)>0):?> <?=max(0,(int)$_GET['before_photos_added'])?> before photo<?=((int)$_GET['before_photos_added']===1?'':'s')?> uploaded.<?php endif;?></div>
<?php if(($_GET['before_photo_error'] ?? '') === '1'):?><div class="card notice-warn"><b>Task saved, but its before photos could not be uploaded.</b> You can add them from the task photo controls.</div><?php endif;?>
