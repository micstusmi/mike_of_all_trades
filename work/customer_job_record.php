...
.customer-find{position:sticky;top:0;z-index:50;background:#edf6fb;border:1px solid #9fc5db;border-radius:12px;padding:12px;margin:12px 0}.customer-find input{width:100%;box-sizing:border-box;padding:11px;border:1px solid #8ca8b8;border-radius:8px;font:inherit}.customer-find-results{display:flex;gap:7px;flex-wrap:wrap;margin-top:8px}.customer-find-results button{border:1px solid #8ca8b8;background:#fff;border-radius:8px;padding:7px 9px;cursor:pointer}.customer-find-empty{color:#66717c;font-size:13px}.customer-page-nav{display:flex;gap:7px;overflow-x:auto;margin:12px 0;padding:9px;background:#fff;border:1px solid #d7e0e6;border-radius:11px;position:sticky;top:0;z-index:49}.customer-page-nav a{flex:0 0 auto;text-decoration:none;color:#17202a;border:1px solid #cbd5dc;border-radius:8px;padding:8px 10px;font-weight:800}.customer-page-nav a.active{background:#17202a;color:#fff}.customer-portal-hub{display:grid;grid-template-columns:repeat(3,1fr);gap:11px;margin:14px 0}.customer-portal-hub a{min-height:75px;display:flex;flex-direction:column;justify-content:space-between;text-decoration:none;color:#17202a;background:#fff;border:1px solid #d7e0e6;border-radius:11px;padding:14px}.customer-portal-hub a strong{font-size:16px}.customer-portal-hub a span{font-size:12px;color:#64717b}.customer-view-hidden{display:none!important}@media(max-width:700px){.customer-portal-hub{grid-template-columns:1fr 1fr}.customer-page-nav{top:0}}
...
<script>(()=>{const input=document.getElementById('customerSectionSearch'),out=document.getElementById('customerSectionResults'),params=new URLSearchParams(location.search),token=params.get('t')||'',selected=params.get('view')||'overview';const raw=[...document.querySelectorAll('.wrap h2')].map((heading,i)=>{const target=heading.closest('.card')||heading;target.id=target.id||'job-section-'+i;return{heading,target,text:(heading.textContent+' '+target.textContent).toLowerCase()}});const sections=raw.filter((s,i,a)=>a.findIndex(x=>x.target===s.target)===i);const key=s=>{const t=s.text;if(/request|task|approval|variation/.test(t))return'tasks';if(/schedule|today/.test(t))return'schedule';if(/photo|progress report/.test(t))return'photos';if(/agreement|pricing|scope/.test(t))return'agreement';if(/time|activity history/.test(t))return'time';if(/invoice|payment|balance|job value|no-charge/.test(t))return'payments';if(/report|update|message/.test(t))return'updates';return'overview'};const pages={overview:'Overview',tasks:'Work & tasks',schedule:'Schedule',photos:'Photos & progress',agreement:'Pricing & agreement',time:'Time & activity',payments:'Invoices & payments',updates:'Updates'};const nav=document.getElementById('customerPageNav');Object.entries(pages).forEach(([k,label])=>{const a=document.createElement('a');a.href=`?t=${encodeURIComponent(token)}&view=${k}`;a.textContent=label;if(k===selected)a.className='active';nav.appendChild(a)});if(selected==='overview'){const hub=document.createElement('div');hub.className='customer-portal-hub';Object.entries(pages).filter(([k])=>k!=='overview').forEach(([k,label])=>{const a=document.createElement('a');a.href=`?t=${encodeURIComponent(token)}&view=${k}`;a.innerHTML=`<strong>${label}</strong><span>Open page →</span>`;hub.appendChild(a)});nav.after(hub);sections.forEach(s=>s.target.classList.add('customer-view-hidden'))}else sections.forEach(s=>s.target.classList.toggle('customer-view-hidden',key(s)!==selected));function render(){const words=input.value.toLowerCase().trim().split(/\s+/).filter(Boolean);out.innerHTML='';if(!words.length)return;const found=sections.filter(s=>words.every(w=>s.text.includes(w))).slice(0,8);if(!found.length){out.innerHTML='<span class="customer-find-empty">No matching section in this job record.</span>';return}found.forEach(s=>{const b=document.createElement('button');b.type='button';b.textContent=s.heading.textContent.trim();b.onclick=()=>{const destination=key(s);if(destination!==selected){location.href=`?t=${encodeURIComponent(token)}&view=${destination}#${s.target.id}`;return}s.target.scrollIntoView({behavior:'smooth',block:'start'})};out.appendChild(b)})}input.addEventListener('input',render);if(location.hash)document.querySelector(location.hash)?.scrollIntoView({block:'start'})})();</script>
</body>
</html>
