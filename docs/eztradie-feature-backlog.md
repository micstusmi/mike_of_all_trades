...
| ALPHA-05 | Quick calendar to draft job | Medium | After core job creation | Start with one connected calendar per business. Exact leading “-EZ ” marker imports a future event into a reviewable draft with title, notes, start/end, timezone and provider event ID. No customer SMS or invoice until owner confirms. Dedupe on business+calendar+event ID; handle edits/cancellations and avoid two-way sync loops. Other busy events may block booking availability without creating jobs. |
