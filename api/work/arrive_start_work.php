...
if($taskId>0){$q=$pdo->prepare("SELECT title FROM work_tasks WHERE id=? AND job_id=? AND status<>'cancelled'");$q->execute([$taskId,$id]);$taskTitle=(string)$q->fetchColumn();if($taskTitle==='')die('Invalid task.');}else{$taskTitle='';}
...
try{
 ...
 $pdo->prepare("INSERT INTO work_sessions (job_id,session_source,worker_id,task_id,started_at,category,start_location,location_detail,billable,notes) VALUES (?,'live',NULL,?,NOW(),'onsite','onsite',?,1,?)")->execute([$id,$taskId?:null,$job['job_address'],$notes]);
...
if($mode==='full_transparency'&&!empty($job['customer_phone'])){
 $msg="Mike of All Trades — Job update\n📍 Mike has arrived at your premises and the travel session has ended. On-site work has now started.".($taskTitle!==''?"\nTask: ".$taskTitle.".":'')."\nLive job record: ".wt_public_url($job);
