...
elseif($action==='add'){
  if($text==='')die('Item required');
  $items=wt_split_customer_request_items($text);
  if(!$items)die('Item required');
  $q=$pdo->prepare("SELECT COALESCE(MAX(item_order),0) FROM work_job_intake_items WHERE job_id=?");$q->execute([$jobId]);$ord=(int)$q->fetchColumn();
  $ins=$pdo->prepare("INSERT INTO work_job_intake_items(job_id,item_order,request_text) VALUES(?,?,?)");
  foreach($items as $item){$ord+=10;$ins->execute([$jobId,$ord,$item]);}
}
