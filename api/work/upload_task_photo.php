...
$mime=(new finfo(FILEINFO_MIME_TYPE))->file($f['tmp_name']);$allowed=['image/jpeg'=>'jpg','image/png'=>'png','image/webp'=>'webp'];if(!isset($allowed[$mime])) fail_photo('Use JPEG, PNG or WEBP.');
...
$stored=$type.'_'.date('Ymd_His').'_'.bin2hex(random_bytes(4)).'.'.(extension_loaded('gd')?'jpg':$allowed[$mime]);
$dest=$dir.'/'.$stored;
try{$storedInfo=wt_store_uploaded_task_photo_file((string)$f['tmp_name'],$dest,$mime,(int)$f['size']);}catch(Throwable $e){fail_photo('Could not preserve the uploaded photo.',500);}
