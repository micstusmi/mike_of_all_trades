...
$jobId=(int)($_POST['job_id']??0);$mid=(int)($_POST['material_id']??0);
if($jobId<=0||$mid<=0)exit('Invalid request');
...
if(!in_array($source,['supplier_purchase','mike_vehicle_stock','customer_supplied','already_on_site','other'],true))$source='supplier_purchase';
...
if(!in_array($reimb,['not_applicable','reimbursement_due','reimbursed','no_reimbursement_due'],true))$reimb='not_applicable';
$notes=trim((string)($_POST['notes']??''));
$legacyCost=$actual??0;
$st=$pdo->prepare("UPDATE work_materials SET task_id=?,material_status=?,supplier=?,estimated_cost=?,actual_cost=?,cost=?,source_type=?,paid_by=?,reimbursement_status=?,notes=?,updated_at=NOW() WHERE id=? AND job_id=?");
$st->execute([$taskId,$status,$supplier,$est,$actual,$legacyCost,$source,$paid,$reimb,$notes,$mid,$jobId]);
