...
- `schema/006_mike_import.sql` and `tools/import_mike.php` stage a **partial** Mike-only migration into an empty business. The importer copies customers, properties, jobs and selected job details, retains old IDs in a private mapping, and reports counts for every deferred `work_*` table. See [MIGRATE_MIKE.md](MIGRATE_MIKE.md). It cannot retire the old tracker.
...
There is deliberately no public registration, automated invitation/recovery email delivery, provider integration or billing. The Mike import only copies the core; tasks, timers, materials, receipts, images, SMS history, payments, invoices, approvals, reports and media are still deferred. It is not a full data migration. The operator CLI token flow is **not** an email verification system until a verified delivery process exists. Login attempts are limited per email and business, but broader abuse controls and audit logging remain. AI reservations have no provider integrations or price schedule yet. No tester accounts or domain routing yet. Continue the route conversion and run two-business abuse tests before deployment.
