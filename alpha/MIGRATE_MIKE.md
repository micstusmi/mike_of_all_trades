...
| `work_tasks`, `work_sessions`, `work_session_breaks` | `alpha_tasks`, `alpha_work_sessions`, `alpha_session_breaks` | Historical tasks and time, including retrospective hour overrides; no live timer operation yet. Source IDs mapped. |
| `work_materials`, `work_payments` | `alpha_materials`, `alpha_payments` | Historical line amounts and notes; any old receipt path is only a reference, not a copied image. Source IDs mapped. Financial totals require separate reconciliation. |
| Every other `work_*` table | Reported as deferred | Records are **not** imported. Source backup must be retained. |

The private owner-only **Migration** tab displays the last inventory and old-to-new counts. Each imported job can display its original scope, Mike job number, tasks, sessions, breaks, materials and payments for inspection. Empty source tables remain in the report too. The report counts rows, not media files; file existence and checksums need a separate audit.
...
5. Run `php alpha/tools/import_mike.php plan BUSINESS_ID`. Check the imported and deferred counts, `invalid_jobs`, `invalid_properties` and all `invalid_relations`. Repair source links in a new source copy if any invalid count is nonzero. Verify the proposed destination is the empty business you just created.
...
- Reconcile imported tasks, sessions and breaks, materials and payments, including financial treatment and billed totals. Import workers, reports, goodwill, receipts and their original images, invoice ledger, approval history, SMS history, job photos and social media records. Map every reference to the new business and job IDs.
